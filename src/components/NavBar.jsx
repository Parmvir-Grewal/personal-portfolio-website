  ...
  return (
    <div className="flex justify-between items-center w-full h-20 px-4 text-white bg-slate-900 fixed z-10 ">
        ...
        <ul className="hidden md:flex">
          {links.map(({id, link})=> (
            <li key={id} className=" cursor-pointer capitalize font-medium text-slate-400"> <Link to={link} smooth duration={500} className='px-4 py-3 mx-2 hover:bg-gradient-to-r from-cyan-500 to-blue-500 hover:text-white rounded-lg hover:scale-105 duration-200'>{link}</Link></li>
          ))}
        </ul>
        ...
        {nav && (
          ...
          justify-center items-center absolute top-0 left-0 w-full h-screen bg-slate-900 text-slate-400">
           {links.map(({id, link})=> (
             <li key={id} className="cursor-pointer capitalize py-3 m-4 text-4xl">
